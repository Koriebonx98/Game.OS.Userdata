...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // If on index page, initialise Steam BPM dashboard after mode is ready
    if (document.getElementById('steamBpmDashboard')) {
        modeReady.then(() => initSteamDashboard());
    }

    // If on account page, apply saved design settings to the UI
    if (document.getElementById('designSettingsSection')) {
        modeReady.then(() => applyDesignSettingsUI());
    }
});
...
/**
 * Build a Steam-style header for the game info modal.
 * Shows hero art, title, play button, stats, and friends tabs.
 */
function _buildSteamInfoHeader(game, platform, title, coverUrl) {
    const heroHtml = coverUrl
        ? `<img class="steam-info-modal-hero" src="${escapeHtml(coverUrl)}" alt="${escapeHtml(title)}"
            onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
           <div class="steam-info-modal-hero-placeholder" style="display:none;">${getPlatformIcon(platform || '')}</div>`
        : `<div class="steam-info-modal-hero-placeholder">${getPlatformIcon(platform || '')}</div>`;
    ...
    if (Array.isArray(typeof _steamActivityCache !== 'undefined' ? _steamActivityCache : null)) {
        ...
        if (sessions.length > 0) {
            const latest = sessions.sort((a, b) =>
                new Date(b.sessionStart || b.loggedAt || 0) - new Date(a.sessionStart || a.loggedAt || 0)
            )[0];
            try {
                const d = new Date(latest.sessionStart || latest.loggedAt);
                lastPlayedStr = d.toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
            } catch (_) {}
            const totalMin = sessions.reduce((acc, e) => acc + (e.minutesPlayed || 0), 0);
            if (totalMin > 0) {
                const hrs = totalMin / 60;
                playTimeStr = hrs >= 1 ? `${hrs.toFixed(1)} hrs` : `${totalMin} min`;
            }
        }
    }

    // Build friends activity (from steam friends cache)
    let friendsHtml = '';
    const friendsForGame = (typeof _steamFriendsCache !== 'undefined' ? _steamFriendsCache : [])
        .filter(f => f.recentGame && f.recentGame.toLowerCase() === (title || '').toLowerCase());
    if (friendsForGame.length > 0) {
        friendsHtml = `
        <div class="steam-info-tab-content active" id="steamInfoTab_activity">
            <div class="steam-info-friends-heading">Friends</div>
            <div style="font-size:0.78em;color:#8f98a0;margin-bottom:10px;">
                ${friendsForGame.length} friend${friendsForGame.length !== 1 ? 's have' : ' has'} played recently
            </div>
            ${friendsForGame.map(f => `
            <div class="steam-info-friend-row">
                <div class="steam-info-friend-avatar">${(f.name || '?')[0].toUpperCase()}</div>
                <div class="steam-info-friend-details">
                    <div class="steam-info-friend-name">${escapeHtml(f.name)}</div>
                    <div class="steam-info-friend-time">${f.minutes > 0 ? (f.minutes / 60).toFixed(1) + ' hrs played recently' : 'Played recently'}</div>
                </div>
            </div>`).join('')}
        </div>
        <div class="steam-info-tab-content" id="steamInfoTab_yourstuff">
            <p style="color:#8f98a0;font-size:0.88em;padding:10px 0;">Your game stats and achievements will appear here.</p>
        </div>
        <div class="steam-info-tab-content" id="steamInfoTab_community">
            <p style="color:#8f98a0;font-size:0.88em;padding:10px 0;">Community content and guides will appear here.</p>
        </div>
        <div class="steam-info-tab-content" id="steamInfoTab_gameinfo">
            <p style="color:#8f98a0;font-size:0.88em;padding:10px 0;">Detailed game information and system requirements below.</p>
        </div>`;
    } else {
        friendsHtml = `
        <div class="steam-info-tab-content active" id="steamInfoTab_activity">
            <p style="color:#8f98a0;font-size:0.88em;padding:10px 0;">No friends have played this recently.</p>
        </div>
        <div class="steam-info-tab-content" id="steamInfoTab_yourstuff">
            <p style="color:#8f98a0;font-size:0.88em;padding:10px 0;">Your game stats and achievements will appear here.</p>
        </div>
    ...
    return `
    <div class="steam-info-modal-body">
        ${heroHtml}
        <div style="padding:16px 18px 0;">
            <div class="steam-info-modal-title">${escapeHtml(title)}</div>
            <div class="steam-info-modal-actions">
                <button class="steam-info-play-btn" onclick="closeGameModal()">▶ Play</button>
            </div>
            <div class="steam-info-modal-stats">
                <div class="steam-info-modal-stat">
                    <span class="steam-info-modal-stat-label">Last Played</span>
                    <span class="steam-info-modal-stat-value">${lastPlayedStr}</span>
                </div>
                <div class="steam-info-modal-stat">
                    <span class="steam-info-modal-stat-label">Play Time</span>
                    <span class="steam-info-modal-stat-value">${playTimeStr}</span>
                </div>
                <div class="steam-info-modal-stat">
                    <span class="steam-info-modal-stat-label">Platform</span>
                    <span class="steam-info-modal-stat-value">${escapeHtml(platform || '—')}</span>
                </div>
            </div>
            <div class="steam-info-tabs">
                <button class="steam-info-tab active" data-tab="activity" onclick="_steamInfoSwitchTab(this,'activity')">Activity</button>
                <button class="steam-info-tab" data-tab="yourstuff" onclick="_steamInfoSwitchTab(this,'yourstuff')">Your Stuff</button>
                <button class="steam-info-tab" data-tab="community" onclick="_steamInfoSwitchTab(this,'community')">Community</button>
                <button class="steam-info-tab" data-tab="gameinfo" onclick="_steamInfoSwitchTab(this,'gameinfo')">Game Info</button>
            </div>
            ${friendsHtml}
            <hr style="border-color:#2a3f5f;margin:14px 0 10px;">
        </div>
    </div>`;
}

/** Switch Steam info modal tab. */
function _steamInfoSwitchTab(btn, tab) {
    const body = btn.closest('.steam-info-modal-body') || document.getElementById('gameModalBody');
    if (!body) return;
    body.querySelectorAll('.steam-info-tab').forEach(t => t.classList.remove('active'));
    body.querySelectorAll('.steam-info-tab-content').forEach(c => c.classList.remove('active'));
    btn.classList.add('active');
    const content = body.querySelector(`#steamInfoTab_${tab}`);
    if (content) content.classList.add('active');
}

/** Bind Steam info tab onclick handlers after modal body is rendered. */
function _bindSteamInfoTabs() {
    const body = document.getElementById('gameModalBody');
    if (!body) return;
    body.querySelectorAll('.steam-info-tab').forEach(btn => {
        btn.onclick = function() { _steamInfoSwitchTab(this, this.dataset.tab); };
    });
}
...
function openGameModal(game, platform) {
    ...
    // Steam game info header (when Design > Game Info = 'steam')
    const gameInfoMode = (typeof getDesignSetting === 'function') ? getDesignSetting('gameInfo') : 'default';
    if (gameInfoMode === 'steam') {
        bodyHtml += _buildSteamInfoHeader(game, platform, title, coverUrl);
    }
    ...
    // Wire Steam info tabs if present
    if (gameInfoMode === 'steam') {
        _bindSteamInfoTabs();
    }
}
...
async function openGameModalFromLibrary(title, platform, titleId) {
    ...
    try {
        ...
        const gameInfoMode = (typeof getDesignSetting === 'function') ? getDesignSetting('gameInfo') : 'default';
        if (gameInfoMode === 'steam') {
            const coverUrl = game ? getGameCoverUrl(game) : null;
            bodyHtml += _buildSteamInfoHeader(source, platform, title, coverUrl);
        }
...
/**
 * Get a design setting value from localStorage.
 * Keys: 'gamesLib' | 'gameInfo'
 * Values: 'default' | 'steam'
 */
function getDesignSetting(key) {
    return localStorage.getItem(`gameOS_design_${key}`) || 'default';
}

/**
 * Set a design setting value and persist it.
 * Called from the Design section in account.html.
 */
...
/**
 * Apply saved design settings to the account page button states.
 * Called on account page load.
 */
function applyDesignSettingsUI() {
    const gamesLib = getDesignSetting('gamesLib');
    const gameInfo = getDesignSetting('gameInfo');
...
const STEAM_DASH_RECENT_COUNT = 6; // Number of recently played games to show in carousel
 ...
 * Shows the dashboard container and loads data.
 */
async function initSteamDashboard() {
    const dashEl = document.getElementById('steamBpmDashboard');
    const stdEl  = document.getElementById('standardDashboard');
    if (!dashEl) return;

    if (!isLoggedIn()) {
        // Not logged in – show standard landing page
        if (stdEl) stdEl.style.display = '';
        dashEl.style.display = 'none';
        return;
    }

    // Show dashboard, hide standard hero
    dashEl.style.display = '';
    if (stdEl) stdEl.style.display = 'none';

    // Hide userDisplay banner and connection status (steam dashboard has its own nav)
    ...
    // Load recent games from activity + library in parallel
    const [activity, library] = await Promise.all([
        _steamLoadActivity(user.username),
        _steamLoadLibrary(user.username)
    ]);

    // Build "recently played" list from activity log
    // Unique games sorted by most recent session
    const seenTitles = new Set();
    const recentFromActivity = activity
        .filter(e => e.type === 'playtime' && e.gameTitle)
        .sort((a, b) => new Date(b.sessionStart || b.loggedAt || 0) - new Date(a.sessionStart || a.loggedAt || 0))
        .filter(e => {
            const key = (e.platform + '||' + e.gameTitle).toLowerCase();
            if (seenTitles.has(key)) return false;
            seenTitles.add(key);
            return true;
        })
        .slice(0, STEAM_DASH_RECENT_COUNT)
        .map(e => {
            // Enrich with cover art from library
            const libEntry = library.find(g =>
                (g.title || '').toLowerCase() === (e.gameTitle || '').toLowerCase() &&
                g.platform === e.platform
            );
            return {
                title:      e.gameTitle,
                platform:   e.platform,
                coverUrl:   libEntry ? (libEntry.coverUrl || null) : null,
                titleId:    libEntry ? (libEntry.titleId || null) : null,
                lastPlayed: e.sessionStart || e.loggedAt || null,
                minutes:    e.minutesPlayed || 0
            };
        });

    // If not enough from activity, pad with library entries (most recently added)
    let recent = [...recentFromActivity];
    if (recent.length < STEAM_DASH_RECENT_COUNT) {
        const activityKeys = new Set(recent.map(r => (r.platform + '||' + r.title).toLowerCase()));
        const libPad = [...library]
            .filter(g => !activityKeys.has((g.platform + '||' + (g.title || '')).toLowerCase()))
            .sort((a, b) => new Date(b.addedAt || 0) - new Date(a.addedAt || 0))
            .slice(0, STEAM_DASH_RECENT_COUNT - recent.length)
            .map(g => ({
                title:      g.title || '',
                platform:   g.platform || '',
                coverUrl:   g.coverUrl || null,
                titleId:    g.titleId || null,
                lastPlayed: g.addedAt || null,
                minutes:    0
            }));
        recent = [...recent, ...libPad];
    }

    _steamCarouselGames = recent;
    _steamCarouselIndex = 0;
    _steamActivityCache = activity;

    _steamRenderCarousel();

    // Load friends
    _steamLoadFriendsSection(user.username);

    // Set up keyboard navigation
    document.addEventListener('keydown', _steamKeyHandler);
}

async function _steamLoadActivity(username) {
    try {
        if (typeof MODE !== 'undefined' && MODE === 'demo') {
            const key  = `gameOS_activity_${username.toLowerCase()}`;
            const data = localStorage.getItem(key);
            return data ? JSON.parse(data) : [];
        }
        return await getActivityLogGitHub(username);
    } catch (_) {
        return [];
    }
}

async function _steamLoadLibrary(username) {
    try {
        if (typeof MODE !== 'undefined' && MODE === 'demo') {
            return typeof getDemoGameLibrary === 'function' ? getDemoGameLibrary(username) : [];
        }
        return typeof getGameLibraryGitHub === 'function' ? await getGameLibraryGitHub(username) : [];
    } catch (_) {
        return [];
    }
}

/** Render the game carousel and show the first game's info. */
function _steamRenderCarousel() {
    const scrollEl  = document.getElementById('steamCarouselScroll');
    const emptyEl   = document.getElementById('steamCarouselEmpty');
    const wrapEl    = document.getElementById('steamCarouselWrap');
    if (!scrollEl) return;

    const games = _steamCarouselGames;

    if (games.length === 0) {
        if (wrapEl)  wrapEl.style.display  = 'none';
        if (emptyEl) emptyEl.style.display = '';
        _steamUpdateInfoPanel(null);
        return;
    }
    ...
    scrollEl.innerHTML = games.map((g, i) => {
        const title  = escapeHtml(g.title || '');
        const plat   = escapeHtml(g.platform || '');
        const icon   = typeof getPlatformIcon === 'function' ? getPlatformIcon(g.platform || '') : '🎮';
        const cover  = g.coverUrl
            ? `<img class="steam-carousel-cover" src="${escapeHtml(g.coverUrl)}" alt="${title}" loading="lazy"
                onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
               <div class="steam-carousel-cover-placeholder" style="display:none;">${icon}</div>`
            : `<div class="steam-carousel-cover-placeholder">${icon}</div>`;
        return `<div class="steam-carousel-item${i === 0 ? ' active' : ''}" tabindex="0"
            data-idx="${i}"
            onclick="_steamSelectGame(${i})"
            onkeydown="if(event.key==='Enter'){_steamSelectGame(${i});}"
            onmouseenter="_steamSelectGame(${i})">
            ${cover}
            <div class="steam-carousel-name" title="${title}">${title}</div>
            <div class="steam-carousel-platform">${plat}</div>
        </div>`;
    }).join('') +
    // "All Games" tile at the end
    `<a href="games.html?tab=library" class="steam-all-games-tile" title="View your full library">
        <span class="steam-all-games-icon">📚</span>
        <span>All Games</span>
    </a>`;

    // Show info for first game
    _steamSelectGame(0);
}

/** Select a game in the carousel by index and update the info panel. */
function _steamSelectGame(idx) {
    const games = _steamCarouselGames;
    if (!games.length) return;
    idx = Math.max(0, Math.min(idx, games.length - 1));
    _steamCarouselIndex = idx;

    // Update active class on items
    const items = document.querySelectorAll('.steam-carousel-item');
    items.forEach((el, i) => el.classList.toggle('active', i === idx));

    // Scroll item into view
    const scrollEl = document.getElementById('steamCarouselScroll');
    if (scrollEl && items[idx]) {
        items[idx].scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' });
    }

    _steamUpdateInfoPanel(games[idx]);
}

/** Update the left-side game info panel. */
function _steamUpdateInfoPanel(game) {
    const emptyEl   = document.getElementById('steamInfoEmpty');
    const detailEl  = document.getElementById('steamInfoDetails');
    const heroImg   = document.getElementById('steamGameHeroImg');
    const heroPlaceholder = document.getElementById('steamGameHeroPlaceholder');
    ...
    if (heroImg && heroPlaceholder) {
        if (game.coverUrl) {
            ...
            heroPlaceholder.textContent = typeof getPlatformIcon === 'function'
                ? getPlatformIcon(game.platform || '') : '🎮';
        }
    }

    const titleEl      = document.getElementById('steamInfoTitle');
    const lastPlayedEl = document.getElementById('steamInfoLastPlayed');
    const playTimeEl   = document.getElementById('steamInfoPlayTime');
    const platformEl   = document.getElementById('steamInfoPlatform');
    ...
    if (lastPlayedEl) {
        if (game.lastPlayed) {
            try {
                ...
                lastPlayedEl.textContent = d.toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
            } catch (_) {
    ...
    // Play time
    if (playTimeEl) {
        if (game.minutes && game.minutes > 0) {
            const hours = game.minutes / 60;
            playTimeEl.textContent = hours >= 1
                ? `${hours.toFixed(1)} hrs`
                : `${game.minutes} min`;
        } else {
            // Sum all activity for this game
            const totalMin = Array.isArray(_steamActivityCache)
                ? _steamActivityCache
                    .filter(e => e.type === 'playtime' &&
                        (e.gameTitle || '').toLowerCase() === (game.title || '').toLowerCase() &&
                        e.platform === game.platform)
                    .reduce((acc, e) => acc + (e.minutesPlayed || 0), 0)
                : 0;
            if (totalMin > 0) {
                const hrs = totalMin / 60;
                playTimeEl.textContent = hrs >= 1 ? `${hrs.toFixed(1)} hrs` : `${totalMin} min`;
            } else {
    ...
    // Friends who played
    const friendsWrapEl = document.getElementById('steamInfoFriendsWrap');
    const friendsListEl = document.getElementById('steamInfoFriendsList');
    if (friendsWrapEl && friendsListEl) {
        const matchingFriends = _steamFriendsCache.filter(f =>
            f.recentGame && f.recentGame.toLowerCase() === (game.title || '').toLowerCase()
        );
        if (matchingFriends.length > 0) {
            friendsWrapEl.style.display = '';
            friendsListEl.innerHTML = matchingFriends.map(f => `
                <div class="steam-friend-row">
                    <div class="steam-friend-avatar">${(f.name || '?')[0].toUpperCase()}</div>
                    <div class="steam-friend-info">
                        <div class="steam-friend-name">${escapeHtml(f.name)}</div>
                        <div class="steam-friend-time">${escapeHtml(f.recentActivity || 'Played recently')}</div>
                    </div>
                </div>`).join('');
        } else {
            friendsWrapEl.style.display = 'none';
        }
    }
}

/** Arrow key handler for carousel navigation. */
function _steamKeyHandler(e) {
    // Only handle arrow keys when the dashboard is visible
    const dash = document.getElementById('steamBpmDashboard');
    if (!dash || dash.style.display === 'none') return;
    const activeTab = document.querySelector('.steam-section.active');
    if (!activeTab || activeTab.id !== 'steamTabGames') return;
...
/** Move carousel by delta (-1 = left, +1 = right). Called by arrow buttons and keyboard. */
function steamCarouselMove(delta) {
    const newIdx = _steamCarouselIndex + delta;
    if (newIdx < 0 || newIdx >= _steamCarouselGames.length) return;
    _steamSelectGame(newIdx);
}

/** Switch the steam dashboard tab. */
function steamSwitchTab(tab) {
    const tabs = ['games', 'friends', 'news', 'recommended'];
    tabs.forEach(t => {
        const navBtn = document.getElementById('steamNav' + t.charAt(0).toUpperCase() + t.slice(1));
        const section = document.getElementById('steamTab' + t.charAt(0).toUpperCase() + t.slice(1));
        if (navBtn) navBtn.classList.toggle('active', t === tab);
        if (section) section.classList.toggle('active', t === tab);
    });
}

/** Load and render the Friends section in the Steam dashboard. */
async function _steamLoadFriendsSection(username) {
    const listEl = document.getElementById('steamFriendsList');
    if (!listEl) return;

    let friendNames = [];
    try {
        if (typeof MODE !== 'undefined' && MODE === 'demo') {
            friendNames = typeof getDemoFriends === 'function' ? getDemoFriends(username) : [];
        } else {
            friendNames = typeof getFriendsGitHub === 'function' ? await getFriendsGitHub(username) : [];
        }
    } catch (_) { friendNames = []; }

    if (!Array.isArray(friendNames) || friendNames.length === 0) {
        listEl.innerHTML = `<div class="steam-friends-empty">
            <p>You have no friends added yet.</p>
            <p><a href="friends.html">Go to Friends →</a></p>
        </div>`;
        return;
    }

    // Load activity for each friend to show recent games
    const friendData = await Promise.all(friendNames.slice(0, 20).map(async name => {
        try {
            let activity = [];
            if (typeof MODE !== 'undefined' && MODE === 'demo') {
                const key  = `gameOS_activity_${String(name).toLowerCase()}`;
                const data = localStorage.getItem(key);
                activity = data ? JSON.parse(data) : [];
            } else if (typeof getActivityLogGitHub === 'function') {
                activity = await getActivityLogGitHub(String(name));
            }
            const latestPlay = activity
                .filter(e => e.type === 'playtime')
                .sort((a, b) => new Date(b.sessionStart || b.loggedAt || 0) - new Date(a.sessionStart || a.loggedAt || 0))[0];
            return {
                name: String(name),
                recentGame: latestPlay ? latestPlay.gameTitle : null,
                recentActivity: latestPlay
                    ? _steamRelativeTime(latestPlay.sessionStart || latestPlay.loggedAt)
                    : null,
                minutes: latestPlay ? (latestPlay.minutesPlayed || 0) : 0
            };
        } catch (_) {
            return { name: String(name), recentGame: null, recentActivity: null, minutes: 0 };
        }
    }));

    _steamFriendsCache = friendData;

    // Re-update info panel now that friends are loaded (in case a game was already selected)
    if (_steamCarouselGames[_steamCarouselIndex]) {
        _steamUpdateInfoPanel(_steamCarouselGames[_steamCarouselIndex]);
    }

    // Render friends list
    const withActivity = friendData.filter(f => f.recentGame);
    const without      = friendData.filter(f => !f.recentGame);

    let html = '';
    if (withActivity.length > 0) {
        html += `<div class="steam-section-header">${withActivity.length} friend${withActivity.length !== 1 ? 's' : ''} played recently</div>`;
        html += withActivity.map(f => `
            <div class="steam-friend-card">
                <div class="steam-friend-card-avatar">${(f.name || '?')[0].toUpperCase()}</div>
                <div class="steam-friend-card-info">
                    <div class="steam-friend-card-name">${escapeHtml(f.name)}</div>
                    <div class="steam-friend-card-activity steam-status-ingame">
                        Playing ${escapeHtml(f.recentGame || '')}
                        ${f.minutes > 0 ? `· ${(f.minutes / 60).toFixed(1)} hrs` : ''}
                    </div>
                </div>
                <div class="steam-friend-card-time">${escapeHtml(f.recentActivity || '')}</div>
            </div>`).join('');
    }
    if (without.length > 0) {
        html += `<div class="steam-section-header" style="margin-top:${withActivity.length ? '16px' : '0'};">Other friends</div>`;
        html += without.map(f => `
            <div class="steam-friend-card">
                <div class="steam-friend-card-avatar">${(f.name || '?')[0].toUpperCase()}</div>
                <div class="steam-friend-card-info">
                    <div class="steam-friend-card-name">${escapeHtml(f.name)}</div>
                    <div class="steam-friend-card-activity steam-status-offline">No recent activity</div>
                </div>
            </div>`).join('');
    }

    listEl.innerHTML = html ||
        `<div class="steam-friends-empty"><p>No friends found.</p><p><a href="friends.html">Manage friends →</a></p></div>`;
}

/** Format a date string as a relative time label (e.g. "2 hrs ago"). */
function _steamRelativeTime(dateStr) {
    if (!dateStr) return '';
    try {
